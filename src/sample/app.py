...
def get_tcp_port() -> int:
    ...
    If the env var PYTHON_APP_TCP_PORT is not present the tcp port is set to the default value of 8080.

    Returns:
        the constructed string"""
    tcp_port: int = 8080
    logger.info('Checking the following environment variable: %s', "PYTHON_APP_TCP_PORT")
    try:
        tcp_port: int = int(os.environ.get("PYTHON_APP_TCP_PORT", tcp_port))
    except ValueError:
        logger.error("Could not parse PYTHON_APP_TCP_PORT env variable %s", exc_info=True)
